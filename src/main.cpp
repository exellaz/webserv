            ...
            for(size_t i = 0; i < pfds.size(); i++) {
                ...
                if (pfds[i].revents & (POLLIN )) {
                    ...
                    else
                    {
                        ...
                        // Find the connection corresponding to the current fd
                        //TODO check the connection fd
                        std::vector<Connection>::iterator connectionFd = CheckConnection(connections, pfds[i].fd);
                        if (connectionFd != connections.end())
                        {
                            int res = receiveClientRequest(connections[i], configs); //pass multiple configs
                            if (res == RECV_CLOSED) {
                                std::cout << "server: socket " << pfds[i].fd << " hung up\n";
